...
# assigns the sample names to each column depending on the presence of the GC column
...
for(i in 1:length(sample.names)){
    ...
    all.exons <- CallCNVs(x = all.exons, transition.probability = trans_prob, chromosome = ExomeCount$chromosome, start = ExomeCount$start, end = ExomeCount$end, name = ExomeCount$exon)
