	...
	def extract(self, path, samplesheet, bed):
		...
		for sample in sample_list:
			if sample not in info:
				info[sample]={}
			command = 'find -L '+path+' -maxdepth 3 -name '+sample+'*.bam ! -name *validation*'
			info[sample]["bam"] = systemcall(command)[0]
			if not bed:
				command = 'find -L '+path+' -maxdepth 3 -name *bed ! -name *genes*'
		...
		for sample in sample_list:
			if sample in sex:
				...
				del info[sample]
		print("#[INFO] SEX dict", sex)
		print(info)
		return info

def __repr__(self):
	return "Test sex:% s sample:% s info:% s" % (self.sex, self.sample, self.info)
...
def canoes(config,output,jobs,dryrun,dag,run):
	...
	command = "/usr/local/lib/miniconda3/bin/snakemake --snakefile Snakefile --configfile " + config + " --directory " + output + " --jobs "+ jobs + snk_opt
	systemcall(command)
...
def checkResults(samplesheet, output, run):
	if not samplesheet:
		sampleID_list = get_sample_id_from_samplesheet(find_any_samplesheet(run))
	else:
		sampleID_list = get_sample_id_from_samplesheet(samplesheet)
	configfile = output+"/analysis.json"
	print("#[INFO] Config ", configfile)
	f = open(configfile, "r")
	conf = json.loads(f.read())
	#sample_analyzed = []
	#for sample_folder in os.listdir(output):
	#	if sample_folder == 'CANOES':
	#		continue
	#else:
	#	sample_analyzed.append(sample_folder)
	sex_raw = ['M','F','A']
	sex_all = {}
	#print(conf["analysis"]["type"])
	#check in analysis.json if SEXUAL chr in enable
	stripped = [typ.strip() for typ in conf["analysis"]["type"]]
	if "F" in stripped and "M" in stripped and "A" in stripped:
		for sex in sex_raw:
			if os.path.exists(osj(output,"CANOES/script_create."+sex+".R")):
				print("#[INFO] script_create."+sex+".R")
				with open(output+"/CANOES/script_create."+sex+".R", "r") as f:
					for lines in f:
						if "sample.names" in lines:
							#print(lines)
							#m = re.search(r"\(([A-Za-z0-9_]+)\)", lines)
							match = lines[lines.find('(')+1:lines.find(')')]
							sex_all["SEX#"+sex] = match.replace('"', '').split(",")
							#print("#[INFO] Sample "+sex, samples_list)
							#ADD Warnings
							break
			else:
				print("#[INFO] No sample SEX#"+sex+" in this analysis")
	#print(sex_all)
	#print(sex_all.values())
	for samples in os.listdir(output):
		if samples in str(sex_all.values()): 
			print("#[INFO] Sample ",samples)
			#CATCH errors in canoes.SEX.log in logs folder
			tsv = osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv")
			#CANOES launched without sexual chromosomes
			if not "SEX#M" and not "SEX#F":
				if os.path.exists(tsv):
					print("#[INFO] TSV ", tsv)
					nodata = "grep 'No data to annotate' "+osj(output, "logs", "canoes.A.log")
					grep_data = systemcall(command)[0]
					if "No data to annotate" in grep_data:
						print("#[INFO] No data to annotate")
						continue
					else:
						if os.path.exists(osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv.tmp")):
							os.remove(osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv.tmp"))
						with open(osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv.tmp"), "w+") as f:
							command = "grep -A1 Error "+osj(output, "logs", "canoes.A.log")
							grep = systemcall(command)[0]
							if "Error" in grep:
								f.write("##Autosomal chromosomes - /!\/!\/!\ A WARNING message appears, please check your results CAREFULLY:"+ grep)
							else:
								f.write("WARNING: Sex is UNKNOWN for Sample "+samples)
								f.write("WARNING: CANOES launched without sexual chromosomes")
								f.write("##Autosomal chromosomes: Samples used for CANOES Analysis :"+str(len(sex_all["SEX#A"]))+"/"+str(len(sampleID_list))+" (recommended 30 samples minimum) : ")
							print(*sex_all["SEX#A"], sep=' ', file=f)
			else:
				if samples in sex_all["SEX#M"]:
					sex_sample = "SEX#M"
				elif samples in sex_all["SEX#F"]:
					sex_sample = "SEX#F"
				#tsv = osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv")
				if os.path.exists(tsv):
					print("#[INFO] TSV ", tsv)
					if os.path.exists(osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv.tmp")):
						os.remove(osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv.tmp"))
					with open(osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv.tmp"), "w+") as f:
						command = "grep -A1 'Error\|Warning' "+osj(output, "logs", "canoes.A.log")
						grep = systemcall(command)[0]
						if "Error" in grep or "Warning" in grep:
							f.write("##Autosomal chromosomes - /!\/!\/!\ A WARNING message appears, please check your results CAREFULLY:"+ grep)
							f.write("\n")
						command2 = "grep -A1 'Error\|Warning' "+osj(output, "logs", "canoes."+sex_sample.split("#")[1]+".log")
						grep2 = systemcall(command2)[0]
						if "Error" in grep2 or "Warning" in grep2:
							f.write("##Sexual chromosomes - /!\/!\/!\ A WARNING message appears, please check your results CAREFULLY:"+ grep2)
							f.write("\n")
						if samples not in sex_all["SEX#M"] and samples not in sex_all["SEX#F"]:
							f.write("WARNING: Sex is UNKNOWN for Sample "+samples)
						else:
							f.write("##Sex for sample "+samples+" is : "+sex_sample+"\n")
						f.write("##Autosomal chromosomes: Samples used for CANOES Analysis :"+str(len(sex_all["SEX#A"]))+"/"+str(len(sampleID_list))+ " (recommended 30 samples minimum) : ")
						print(*sex_all["SEX#A"], sep=' ', file=f)
						#f.write("\n")
						f.write("##Sexual chromosomes - Samples used for CANOES Analysis with "+sex_sample+" : "+str(len(sex_all[sex_sample]))+ "(recommended 30 samples minimum) : ")
						print(*sex_all[sex_sample], sep=' ', file=f)
					
				else:
					print("#ERROR No TSV generated for sample "+ samples)
					
	#copy errors message in file and keep old tsv in log in folder (good things I guess)
	folder_log_tsv = osj(output, "logs", "TSV")
	folder_log = osj(output, "logs")
	if not os.path.exists(folder_log_tsv):
		os.mkdir(folder_log_tsv)
	print("#[INFO] Add Errors | Warning | INFO Messages ...")
	#Samples F and samples M
	for samples in os.listdir(output):
		if samples in str(sex_all.values()):
			print("#[INFO] Sample ", samples)
			tsv_tmp = osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv.tmp")
			tsv = osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv")
			with open (tsv) as f:
				with open (tsv_tmp, "a") as final:
					for lines in f:
						final.write(lines)
			shutil.move(osj(output, samples, "CANOES" , samples+".canoes.annotsv.tsv"),osj(folder_log_tsv, samples+".canoes.annotsv.tsv"))
			os.rename(tsv_tmp, osj(output, samples, "CANOES", samples+".canoes.annotsv.tsv"))
			
		#allsample
		elif samples == "CANOES":
			if os.path.exists(osj(output, "CANOES", "all.canoes.annotsv.sorted.tsv")):
				print("#[INFO] Process all canoes TSV ...")
				tsv_tmp = osj(output, "CANOES", "all.canoes.annotsv.sorted.tsv.tmp")
				tsv = osj(output, "CANOES", "all.canoes.annotsv.sorted.tsv")
				if not os.path.exists(osj(folder_log, "canoes.A.log")):
					print("ERROR CANOES may have a problem EXIT")
					exit 
				elif os.path.exists(osj(folder_log, "canoes.M.log")) == False and os.path.exists(osj(folder_log, "canoes.F.log")) == False:
					with open (tsv_tmp, "w+") as f:
						f.write("##Sexual chromosomes - /!\/!\/!\ A WARNING message appears, please check your results CAREFULLY: CANOES launched without sexual chromosomes")
						f.write("\n")
				else:
					for log in os.listdir(folder_log):
						if log == "canoes.A.log":
							cmd = "grep -A1 'Error\|Warning' "+osj(folder_log, log)
							grep = systemcall(cmd)[0]
							with open(tsv_tmp, "w+") as f:
								if "Error" in grep or "Warning" in grep:
									print("#[INFO] grep:", grep)
									f.write("##Autosomal chromosomes - /!\/!\/!\ A WARNING message appears, please check your results CAREFULLY:"+ grep)
									f.write("\n")
								else: 
									f.write("##Autosomal chromosomes: Samples used for CANOES Analysis :"+str(len(sex_all["SEX#A"]))+"/"+str(len(sampleID_list))+ " (recommended 30 samples minimum) : ")
									print(*sex_all["SEX#A"], sep=' ', file=f)
						elif log == "canoes.F.log" or log == "canoes.M.logs":
							cmd = "grep -A1 'Error\|Warning' "+osj(folder_log, log)
							grep = systemcall(cmd)[0]
							with open(tsv_tmp, "w+") as f:
								if "Error" or "Warning" in grep:
									f.write("##Sexual chromosomes - /!\/!\/!\ A WARNING message appears, please check your results CAREFULLY")
									f.write("\n")
								else:
									f.write("##Sexual chromosomes - The analysis ENDED WELL!")
									f.write("\n")
						#elif not os.path.exists(osj(output,"logs","canoes.F.log")) and not os.path.exists(osj(output,"logs","canoes.M.log")):
					...
					with open (tsv) as f:
						with open (tsv_tmp, "a") as final:
							for lines in f:
								final.write(lines)
				shutil.move(tsv,osj(folder_log, "all.canoes.annotsv.sorted.tsv"))
				os.rename(tsv_tmp, osj(output,"CANOES", "all.canoes.annotsv.sorted.tsv"))
	print("#[INFO] Check Results DONE")
...
def main_run(args):
	...
	checkResults(args.samplesheet, args.output, args.run)
...
if __name__ == '__main__':
	#run = Run("/STARK/data/output_validation-run/results/DIAG_RUN_TEST","/STARK/data/output_validation-run/samplesheet.csv","/STARK/data/output_validation-run/results/DIAG_RUN_TEST/SGT140392/SGT140392.bed")
	#print(run)	
	main()