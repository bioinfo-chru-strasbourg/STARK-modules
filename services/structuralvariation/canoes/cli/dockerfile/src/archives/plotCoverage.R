...
PlotCoverage <- function(SAMPLE, INPUT, OUTPUT){
  ...
  #print(head(my_data))
  my_data$chr = str_extract(my_data$Genomic_Region, "[a-z]*[0-9]*")
  y_data.long$chr = str_extract(my_data$Genomic_Region, "[a-z]*[0-9]*")
  #print(head(my_data))
	#do boxplot
  
  boxplots = list()
  chromosomes = unique(my_data$chr)
  print(head(y_data.long))
	print(chromosomes)
  for (chrom in chromosomes) {
	#print(paste("INFO ", chrom))
	#print(head(y_data.long))
	#test = subset(y_data.long, chr == chromosomes[[chrom]])
	#print(tail(test))

	boxplots[[chrom]] =  ggplot() + 
	geom_boxplot(data=y_data.long %>% filter(chr == chrom), aes(x=factor(Genomic_Region, levels=lab), y=value)) +
	geom_point(data=my_data %>% filter(chr == chrom), aes(x=factor(Genomic_Region, levels=lab), y=Sample), shape=17, color="green") +
	...
	#png(type="cairo", res=300)
	#ticks <- length(aes(x=factor(Genomic_Region, levels=lab)))
	#pdf(paste(OUTPUT), width = nrow(my_data))
	#print(nrow(my_data))
	#dev.off()
	OUT = paste(OUTPUT, chrom, 'pdf', sep=".")
	print(OUT)
	wid = nrow(my_data %>% filter(chr == chrom))
	#print(length(wid))
	#print(wid)
	ggsave(boxplots[[chrom]], file=OUT, width = wid ,  limitsize = FALSE, device = "pdf")
	#return(my_data)
	}
  #dev.off()
}
