...
EstimateVariance <- function(counts, ref.sample.names, sample.weights){
  ...
  fit <- gam(var ~ s(mean) + s(gc), family=Gamma(link=log), method="REML", data=counts.subset)
  
  ###############
  #Dev 
  ###############
  #gam.check(fit, pch=19, cex=0.3)
  
 #pdf(file=paste("C:/Users/LAMOUCHJ/Documents/CANOES/test/", ref.sample.names, '.plots.pdf'))
 #
 #qplot(vis.gam(fit), main="vis.gam smooth parameters")
 #qplot(gam.check(fit), main="gam.check smooth parameters")
 #
 #dev.off()
 #
 #vis.gam(fit)
  #jpeg(file=paste("C:/Users/LAMOUCHJ/Documents/CANOES/test/", ref.sample.names, '.gam-check.jpeg'))
