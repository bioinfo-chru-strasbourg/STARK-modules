...
def copyAllResults(output, exclude):
	sample_list = []
	...
	for sample in sample_list:
		if isValidSample(sample, cleanList(exclude)):
			if not os.path.isdir(osj(output,sample,"CANOES/")):
				os.mkdir(osj(output,sample,"CANOES/"))
	...
	# if os.path.exists(output+"/Rplots.pdf"):
		# os.remove(output+"/Rplots.pdf")
	# if os.path.exists(output+"/.RData"):
		# os.remove(output+"/.RData")
...
def main_sample(args):
	...
	copyAllResults(args.output, args.exclude)
