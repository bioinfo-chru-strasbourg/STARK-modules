...
# Usage 
function usage {
	echo "# USAGE: $(basename $0) --input=<RUN_INPUT>,<RUN_INPUT>,... --nbDaysBack=<DAYS_BACK> --genome=<GENOME_FOLDER> --canoes=<CANOES_CONTAINER>";
	echo "# -i/--input 		INPUT directory where runs are";
	echo "# -t/--nbDaysBack	Analyze run OLDER than X days";
	echo "# -g/--genome		GENOME folder";
	echo "# -c/--canoes		CANOES container name and port";
...
ARGS=$(getopt -o "i:t:g:c:h" --long "input:,nbDaysBack:,genome:,canoes:,help" -- "$@" 2> /dev/null)
if [ $? -ne 0 ]; then
	:
fi;
PARAM=$@
...
for RUN in $RUN_NAME; do
	echo $RUN
	if [[ -z `grep ${RUN} /home1/L_PROD/NGS/BIO_INFO/BIO_INFO_Victor/Projets/Listener/Analyzed_runs.txt` ]]; then
	...
	if echo $MICROSERVICE | grep -q "CANOES"; then
		curl 'docker run -dti --name=service-canoes -v ${GROUP_INPUT}/${RUN}:${GROUP_INPUT}/${RUN} -v ${GENOME}:${GENOME} canoes:1.1 /bin/bash /app/bin/canoes run -r ${GROUP_INPUT}/${RUN} -s ${SAMPLESHEET} -l ${BED} -e A,F,M -g ${GENOME}/hg19.fa -o ${GROUP_INPUT}/${RUN}/CANOES' ${CANOES}
		while [ "$( docker container inspect -f '{{.State.Status}}' service-canoes )" == "running" ]; do
			sleep 5m
		done
		docker rm -f service-canoes
	echo $RUN >> /home1/L_PROD/NGS/BIO_INFO/BIO_INFO_Victor/Projets/Listener/Analyzed_runs.txt
	fi
done
