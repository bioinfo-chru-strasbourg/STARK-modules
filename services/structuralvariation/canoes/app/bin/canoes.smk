...
# Design no annotation all samples 
rule correct_1based:
...
rule add_chr_to_vcf:
	...
	output: temp(f"{resultDir}/{serviceName}.{date_time}.allsamples.{{aligner}}.Design_chr.vcf")
	shell: "awk -F '\\t' 'BEGIN {{OFS=\"\\t\"}} /^##/ {{print}} /^#CHROM/ {{print}} /^[^#]/ {{$1 = \"chr\" $1; print}}' {input} > {output}"

rule fix_vcf:
	input: rules.add_chr_to_vcf.output
	output: 
			vcfgz=temp(f"{resultDir}/{serviceName}.{date_time}.allsamples.{{aligner}}.Design_unorm.vcf.gz"),
			vcfgztbi=temp(f"{resultDir}/{serviceName}.{date_time}.allsamples.{{aligner}}.Design_unorm.vcf.gz.tbi")
