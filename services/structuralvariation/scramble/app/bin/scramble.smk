...
# Priority order
ruleorder: copy_bam > copy_cram > cramtobam > indexing > split_tsv > correct_tsv > AnnotSV
...
rule all:
	...
	input:
		...
		[f"{resultDir}/{serviceName}.{date_time}.allsamples.{{aligner}}.AnnotSV.Full.tsv"] +
		...
		([f"{resultDir}/{serviceName}.{date_time}.allsamples.{{aligner}}.AnnotSV.Design.tsv"] if config['BED_FILE'] else []) +
...
rule scramble:
	...
	output:
		temp(f"{resultDir}/{{sample}}/{serviceName}/{{sample}}_{date_time}_{serviceName}/{serviceName}.{date_time}.{{sample}}.{{aligner}}.raw.vcf")
	log: 
...
rule merge_vcf:
	...
	output:f"{resultDir}/{serviceName}/{serviceName}.{date_time}.allsamples.{{aligner}}.Full.vcf.gz"
	shell: "if [ {sample_count} -eq 1 ]; then cp {input} {output}; else bcftools merge {input} -O z -o {output} 2> {log}; fi; tabix {output}"	

# filter raw full with bed to get design
rule filter_vcf:
	...
	log: f"{resultDir}/{serviceName}.{date_time}.allsamples.{{aligner}}.Design.bedtoolsfilter.log"
...
rule AnnotSV:
	...
	"""
	input: rules.vcf_normalisation.output
	output: f"{resultDir}/{serviceName}/{serviceName}.{date_time}.allsamples.{{aligner}}.AnnotSV.Design.tsv"
	log: f"{resultDir}/{serviceName}/{serviceName}.{date_time}.allsamples.{{aligner}}.AnnotSV.Design.log"
	params:
...
rule correct_tsv:
	input: rules.AnnotSV.output
	output: temp(f"{resultDir}/{serviceName}/{serviceName}.{date_time}.allsamples.{{aligner}}.AnnotSVcorr.Design.tsv")
	shell: 
		""" 
...
rule correct_chr:
	...
	output: temp(f"{resultDir}/{serviceName}/{serviceName}.{date_time}.allsamples.{{aligner}}.AnnotSVcorr_chr.Design.tsv")
	shell:
		"""
