	...
	while (<BIN_FILE>){
		...
		if (scalar @rows_CDS == 0){						# no CDS found for this coords
			if (@rows_no_CDS == 0){ 					#coord not in db
				...
				$median = "";
				$seqs_rep = "";
				$ensg="";$gene_name="";$gene_desc="";$ensfm="";$fam_desc="";$gene_status="";$fam_size="";$kaks_mouse="";$kaks_macaque="";$mim_status="";$freq_av="";$freq_ceu="";
                                push @coord_arr_no_CDS, "$chr\t$coord1\t$coord2\t$orn_usr\t$alleles\t$usr_comment";
                                $result = "$rsid\t$enst\t$ensp\t$region\t$snp_type\t$codon1\t$codon2\t$AA1\t$AA2\t$AAPOS2\t$score\t$median\t$seqs_rep\t$ensg\t$gene_name\t$gene_desc\t$ensfm\t$fam_desc\t$gene_status\t$fam_size\t$kaks_mouse\t$kaks_macaque\t$mim_status\t$freq_av\t$freq_ceu";
                                $coord_result_no_CDS_hash{"$chr\t$coord1\t$coord2\t$orn_usr\t$alleles\t$usr_comment"} = $result;
			}
				...
				$seqs_rep = "";
				$ensg=@elts_no_CDS[26];
				$gene_name =@elts_no_CDS[27]; 
				$gene_desc =@elts_no_CDS[28];
				$ensfm =@elts_no_CDS[29];
				$fam_desc =@elts_no_CDS[30];
				$gene_status =@elts_no_CDS[31];
				$fam_size =@elts_no_CDS[32];
				$kaks_mouse =@elts_no_CDS[33];
				$kaks_macaque =@elts_no_CDS[34];
				$mim_status =@elts_no_CDS[35];
				$freq_av =@elts_no_CDS[36];
				$freq_ceu =@elts_no_CDS[37];
                                push @coord_arr_no_CDS, "$chr\t$coord1\t$coord2\t$orn_db\t$alleles\t$usr_comment";
                                $result = "$rsid\t$enst\t$ensp\t$region\t$snp_type\t$codon1\t$codon2\t$AA1\t$AA2\t$AAPOS2\t$score\t$median\t$seqs_rep\t$ensg\t$gene_name\t$gene_desc\t$ensfm\t$fam_desc\t$gene_status\t$fam_size\t$kaks_mouse\t$kaks_macaque\t$mim_status\t$freq_av\t$freq_ceu";
                                $coord_result_no_CDS_hash{"$chr\t$coord1\t$coord2\t$orn_db\t$alleles\t$usr_comment"} = $result;
			...
			if($selected_row != -1){			#valid CDS
				...
				$score = @elts_CDS[23];
                                $median = @elts_CDS[24];
                                $seqs_rep = @elts_CDS[25];
				$ensg = @elts_CDS[26];
				$gene_name =@elts_CDS[27];
                                $gene_desc =@elts_CDS[28];
                                $ensfm =@elts_CDS[29];
                                $fam_desc =@elts_CDS[30];
                                $gene_status =@elts_CDS[31];
                                $fam_size =@elts_CDS[32];
                                $kaks_mouse =@elts_CDS[33];
				$kaks_macaque =@elts_CDS[34];
                                $mim_status =@elts_CDS[35];
                                $freq_av =@elts_CDS[36];
                                $freq_ceu =@elts_CDS[37];


                                push @coord_arr_CDS, "$chr\t$coord1\t$coord2\t$orn_db\t$alleles_db\t$usr_comment";
                                $result = "$rsid\t$enst\t$ensp\t$region\t$snp_type\t$codon1\t$codon2\t$AA1\t$AA2\t$AAPOS2\t$score\t$median\t$seqs_rep\t$ensg\t$gene_name\t$gene_desc\t$ensfm\t$fam_desc\t$gene_status\t$fam_size\t$kaks_mouse\t$kaks_macaque\t$mim_status\t$freq_av\t$freq_ceu";
                                $coord_result_CDS_hash{"$chr\t$coord1\t$coord2\t$orn_db\t$alleles_db\t$usr_comment"} = $result;
                        }
				...
				push @coord_arr_CDS, "$chr\t$coord1\t$coord2\t$orn_db\t$nt1\/$nt1\t$usr_comment";
        	                $result = "$rsid\t$enst\t$ensp\t$region\tSynonymous\t$codon1\t$codon1\t$AA1\t$AA1\t$AAPOS2\t$score\t$median\t$seqs_rep\t$ensg\t$gene_name\t$gene_desc\t$ensfm\t$fam_desc\t$gene_status\t$fam_size\t$kaks_mouse\t$kaks_macaque\t$mim_status\t$freq_av\t$freq_ceu";
	                        $coord_result_CDS_hash{"$chr\t$coord1\t$coord2\t$orn_db\t$nt1\/$nt1\t$usr_comment"} = $result;

			}
			elsif($AA1_VALID_found == 0){		#All ensts found bad. save this coord in a different file.
				print BAD_ENST_FILE "$chr\t$coord1\t$coord2\t$orn_db\t$nt1\/$nt2\t$usr_comment\n";
				next;
			}
		}
	}
...
foreach $coord (@coord_arr_CDS) {
	...
	if (exists ($hash_enst_aapos_seen{"$enst\t$pos"})){
		...
		$enst_line          = "$coord\t$enst\t$aa1$pos$aa2\t$ensp\t$rsid\t$region\t$codon1-$codon2\t$snp_type\t$score\t$median\t$seqs_rep\t$ensg\t$gene_name\t$gene_desc\t$ensfm\t$fam_desc\t$gene_status\t$fam_size\t$kaks_mouse\t$kaks_macaque\t$mim_status\t$freq_av\t$freq_ceu";
...
foreach $coord (@coord_arr_no_CDS) {
	...
	if ($aa1 eq "NA"){
        ...
        $enst_line          = "$coord\t$enst\t$subst\t$ensp\t$rsid\t$region\t$codon1-$codon2\t$snp_type\t$score\t$median\t$seqs_rep\t$ensg\t$gene_name\t$gene_desc\t$ensfm\t$fam_desc\t$gene_status\t$fam_size\t$kaks_mouse\t$kaks_macaque\t$mim_status\t$freq_av\t$freq_ceu";	
        push( @enst_pred_list, $enst_line );
	print ENSTFILE "$enst_line\n";
}
close(ENSTFILE);
...
$heading = get_output_heading($output_options);
open( OUTFILETABLE, ">$tmp/$pid.combined_siftresults.predictions.table.html" );
open( OUTFILETSV,   ">$tmp/$pid.combined_siftresults.predictions.tsv" );
...
while (<ENST_PRED_FILE>) {
	...
	for $cell (@fields) {
		...
		if ( $cell =~ /DAMAGING/ || $cell =~ /Warning/ ) {
			...
			print OUTFILETABLE "<td><A HREF=\"http:\/\/www.ncbi.nlm.nih.gov\/sites\/entrez?db=snp&cmd=search&term=\+$dbsnp_id\" target=\"_blank\">$dbsnp_id:$mutation</A></td>";
		}
		elsif($count >= 9 && $count <=12 && $is_synonymous == 1){
			print OUTFILETABLE "<td>N\/A</td>";
		}
		else {
...
open(TSV_COMBINED,">$tmp/$master_pid.combined_siftresults.predictions.tsv")|| die ("Cannot open prediction tsv files for combining");
open(HTML_COMBINED,">$tmp/$master_pid.combined_siftresults.predictions.table.html")||die("Cannot open prediction tsv files for combining");
print TSV_COMBINED "$heading\n";
print HTML_COMBINED '<table border="1" cellspacing="0" cellpadding="4">';
print HTML_COMBINED '<thead>';
print HTML_COMBINED '<tr><th>';

print HTML_COMBINED "$heading</th></tr>\n";


if ($last_partition == 1){
	for ($i = $master_pid + 1; $i <= $pid; $i ++){
		open(TSV,"$tmp/$i.combined_siftresults.predictions.tsv")|| die ("Cannot open prediction tsv files for combining");
		open(HTML,"$tmp/$i.combined_siftresults.predictions.table.html")|| die ("Cannot open prediction tsv files for combining");
		while (<TSV>){
			#combined TSV file
			chomp;
			if ($_ =~ /Coordinates/i){
			...
			$count = 0;
				...
				$count ++;
                        	if ( $cell =~ /DAMAGING/ || $cell =~ /Warning/ ) {
                                	print HTML_COMBINED "<td><font color=red>$cell</font></td>";
                        	}
                        	elsif ( $cell =~ /NOT PREDICTED/i || $cell =~ /not found/i ) {
                                	print HTML_COMBINED "<td><font color=blue>$cell</font></td>";
                        	}
				elsif ($cell =~ /(rs\d+):(.+?)/i){
                                	$dbsnp_id = $1;$mutation = $2;
                                	print HTML_COMBINED "<td><A HREF=\"http:\/\/www.ncbi.nlm.nih.gov\/sites\/entrez?db=snp&cmd=search&term=\+$dbsnp_id\" target=\"_blank\">$dbsnp_id:$mutation</A></td>";
                        	}
				elsif($count >= 9 && $count <=12 && $is_synonymous == 1){
		                        print HTML_COMBINED "<td>N\/A</td>";
                		}
	...
	update_stats($master_pid,$CDS_num,$tolerated_num,$damaging_num,$nonsynonymous_num,$synonymous_num,$novel_num);
}
close (TSV_COMBINED);
close (HTML_COMBINED);
...
"Dear User\n\nThank you for using SIFT.\n\nPlease find the results of your recent query attached with this message.\nRemember this job id \"$master_pid\" for any future correspondance.\nDo not hesitate to contact us if you have any questions about SIFT.\n\nThanks\nSIFT Team\nJ Craig Venter Institute (West Coast Campus)\n10355 Science Center Drive\nSan Diego, CA 92121\nUSA";
	close(MESSAGE);
	system(
"mutt -F /opt/www/sift/htdocs/.muttrc -a $tmp/$master_pid.combined_siftresults.predictions.tsv -s \"SIFT Results for Job ID $master_pid\" $address <$tmp/$master_pid.email_message.txt"
	);
}
sub update_stats{
	...
	$novel_pct = int($novel_num * 100 / $predicted_num);
	...
	while (<OUTPAGE>){
		chomp;
		if ($_ =~ /Number of coding variants/){
			print OUTPAGE_MODIFIED "Number of coding variants: $CDS_num <BR>\n";
		}
		elsif($_ =~ /Coding variants predicted/){
			print OUTPAGE_MODIFIED "Coding variants predicted: $predicted_pct% ($predicted_num) <BR>\n";
		}
		elsif($_ =~ /Tolerated/){
                        print OUTPAGE_MODIFIED "Tolerated: $tolerated_pct% ($tolerated_num) <BR>\n";
                }
		elsif($_ =~ /Damaging/){
                        print OUTPAGE_MODIFIED "Damaging: $damaging_pct% ($damaging_num) <BR>\n";
                }
		elsif($_ =~ /Nonsynonymous/){
                        print OUTPAGE_MODIFIED "Nonsynonymous: $nonsynonymous_pct% ($nonsynonymous_num) <BR>\n";
                }
		elsif($_ =~ /Synonymous/){
                        print OUTPAGE_MODIFIED "Synonymous: $synonymous_pct% ($synonymous_num) <BR>\n";
                }
		elsif($_ =~ /Novel/){
                        print OUTPAGE_MODIFIED "Novel: $novel_pct% ($novel_num) <BR>\n";
                }
		else{
	...
	while (<OUTPAGE>){
        	...
        	if ($_ =~ /.+?(Partitioned set .+?)\<\/td\>\<td\>(.+?)\<.+?\>$slave_pid\<.+?/){
	                ...
	                if ($status =~ /running/i){
	                        ...
	                        print OUTPAGE_MODIFIED "<tr><td>$partition</td><td>$job_size</td><td>$slave_pid</td><td><font color=green>Complete</font></td><td><a href=/sift-bin/catfile.csh?$tmp/$slave_pid.combined_siftresults.predictions.table.html target=\"_blank\">$slave_pid table</a></td><td><A HREF=/tmp/$slave_pid.combined_siftresults.predictions.tsv>$slave_pid results</A></td></tr>\n";
	                }
	        }
			...
			$partition = "Complete set";
                        ...
                        if ($status =~ /running/i){
                                ...
                                print OUTPAGE_MODIFIED "<tr BGCOLOR=\"\#DCDCDC\"><td>$partition</td><td>$job_size</td><td>$slave_pid</td><td><font color=green>Complete</font></td><td><a href=/sift-bin/catfile.csh?$tmp/$slave_pid.combined_siftresults.predictions.table.html target=\"_blank\">$slave_pid table</a></td><td><A HREF=/tmp/$slave_pid.combined_siftresults.predictions.tsv>$slave_pid results</A></td></tr></table>\n";
                        }
                }
...
"Coordinates\tCodons\tTranscript ID\tProtein ID\tSubstitution\tRegion\tdbSNP ID\tSNP Type\tPrediction\tScore\tMedian Info\t\# Seqs at position";
        @options = ("Gene ID","Gene Name","Gene Desc","Protein Family ID","Protein Family Desc","Transcript Status","Protein Family Size","Ka/Ks (Mouse)","Ka/Ks (Macaque)","MIM Morbid Status","Average Allele Freqs","CEU Allele Freqs");
...
"$chrom,$coords,$orn,$alleles\t$codons\t$proteinid\t$ensp\t$subst\t$region\t$rsid\t$snp_type\t$pred\t$score\t$median\t$seqs_rep";
	...
	for ($i = 0 ; $i < scalar @elts2; $i++){
                if (@elts2[$i] eq "1"){
                        if ($i eq "7" || $i eq "8") {# format Ka/Ks to 2 decimal places.
                                $kaks = sprintf("%.3f",@elts[16+$i]);
                                $table_row.= "\t$kaks";
                        }
                        else{
                                $table_row.= "\t@elts[16+$i]";
