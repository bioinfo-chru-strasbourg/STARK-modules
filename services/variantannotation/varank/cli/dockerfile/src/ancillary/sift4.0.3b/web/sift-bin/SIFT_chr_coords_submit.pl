...
# reset everything to synonmyous
				 ...
				 $row = @rows_CDS[$ref_allele_row];
                                 ...
                                 my $v = make_variant_with_elts ($row);
# reset everything to synonmyous
				my $error_message = "<a href=\"/SIFT_help.php#SIFT_OUTPUT_GENOME_Error\" rel=\"external\">Gene Annotation Error</a>";
                                $v->snp_type ("Unknown");
                                $v->codon1 ("");
                                $v->codon2 ("");
                                $v->AA1 ("NA");
	...
	while (<OUTPAGE>){
        	...
        	if ($_ =~ /.+?(Partitioned set .+?)\<\/td\>\<td\>(.+?)\<.+?\>$slave_pid\<.+?/){
	                ...
	                if ($status =~ /running/i){
	                        ...
	                        print OUTPAGE_MODIFIED "<tr class=\"tableRow".($outpage_counter++ % 2 == 0 ? "Even" : "Odd")."\"><td><p>$partition</p></td><td>$job_size</p></td><td><p>$slave_pid</p></td><td><p class=\"green\">Complete</p></td><td><p><a href=\"/sift-bin/catfile.csh?$tmp/$slave_pid\_predictions.html\" rel=\"external\">$slave_pid table</a></p></td><td><p><a href=\"/tmp/$slave_pid\_predictions.tsv\">$slave_pid results</a></p></td></tr>\n";
	                }
	        }
			...
			$partition = "Complete set";
                        ...
                        if ($status =~ /running/i){
                                ...
                                print OUTPAGE_MODIFIED "<tr class=\"tableRow".($outpage_counter++ % 2 == 0 ? "Even" : "Odd")."\"><td><p>$partition</p></td><td>$job_size</p></td><td><p>$slave_pid</p></td><td><p class=\"green\">Complete</p></td><td><p><a href=\"/sift-bin/catfile.csh?$tmp/$slave_pid\_predictions.html\" rel=\"external\">$slave_pid table</a></p></td><td><p><a href=\"/tmp/$slave_pid\_predictions.tsv\">$slave_pid results</a></p></td></tr></table>\n";
                        }
                }
