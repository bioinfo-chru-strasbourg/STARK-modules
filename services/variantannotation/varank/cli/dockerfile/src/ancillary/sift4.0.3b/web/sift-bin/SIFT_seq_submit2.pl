...
if ( $ip_counts > $upper_limit ) {
	print
"<H1>Your computer has exceeded its daily limit for sequence submissions.</H1><BR>";
	print
"Please try  <A HREF=\"/www/SIFT_BLink_submit.html\">SIFT BLink</A HREF> to get your predictions or <A HREF=\"/sift-bin/contact.pl\">contact</A HREF> us so that we can help you.  Thank you for using SIFT. <BR>";
	exit;
}
...
print
"Your job id is $pid and is currently running. <BR><BR> If your browser times out before results are shown, please go to http://sift.jcvi.org/sift-bin/format.pl?$pid in <b>20 minutes</b>.  Opening it up before 20 minutes will throw an error (in which case, just refresh). <BR><BR> Problems? Contact <A HREF=\"contact.pl\">us<A> with your job id.\n";
...
#system("./add_queue_entry.pl SIFT_queue $program_call $pid query_seq $queryseqfile $out  $info $subst_file 0 $seq_identity_filter $names{database} $address > $comments 2> /dev/null");
	system(
"$program_call $pid query_seq $queryseqfile $out  $info $subst_file 0 $seq_identity_filter $names{database} $address > $comments 2> /dev/null"
	);
}
else {

#system("echo $program_call $pid query_seq $queryseqfile $out $info $subst_file 0  $seq_identity_filter $names{database} $address > $tmp/remove");
	system(
"$program_call $pid query_seq $queryseqfile $out $info $subst_file 0  $seq_identity_filter $names{database} $address > $comments 2>&1"
	);
}
...
print "</PRE>";
print
"Your results will be available <A HREF=\"/sift-bin/format.pl?$pid\">here</A> for the next 24 hours.<BR>";
...
print
"<b>$seqno</b> sequences were selected to be closely related to your query sequence.<BR>\n";

print
"<A HREF=\"/sift-bin/catfile.csh?$tmp/$pid.msf+Alignment+PRE\" TARGET=_blank>";
print "PSIBLAST alignment of submitted sequences</A><BR>\n";

print
"<A HREF=\"/sift-bin/catfile_notitle.csh?$tmp/$pid.alignedfasta+PRE\" TARGET=_blank>Alignment in FASTA format </A> (for modification)<BR>";

print_psiblast_alignment_desc();
print
"<i>Please check the sequences that have been chosen.  If the sequences are too diverged from your query or the alignment is questionable, we suggest you modify the fasta-formatted file above and <A HREF=\"/www/SIFT_aligned_seqs_submit.html\" TARGET=_blank>resubmit</A>.</i><BR>";
...
#	print "<b>ERROR!</b> Please <A HREF=\"http://blocks.fhcrc.org/contact.html\" TARGET=blank> email us </A> with your sequence if you do not understand the error. <BR><BR>";
	...
	if ( open( OUT, "$tmp/$pid.seq.query.globalX" ) ) {
		...
		print
"<A HREF=\"/pauline-bin/catfile.csh?btest-tmp/$pid.aln+Alignment+PRE\">";
...
#email the results
$num_tables = `ls $tmp/$$.aatable*.html | wc -l`;
if ( $address ne "" ) {
...
"mutt -F /opt/www/sift/htdocs/.muttrc -a $tmp/$pid.alignedfasta -a $tmp/$pid.siftresults.matrix.html -a $tmp/$pid.siftresults.predictions.html";
	...
	#find how many tables and attach
	for ( $i = 1 ; $i <= $num_tables ; $i++ ) {
		$mutt_command .= " -a $tmp/$pid.aatable$i.html";
	}
	$mutt_command .=
" -s \"SIFT Results for Job ID $pid\" $address <$tmp/$pid.email_message.txt";
	#system("$mutt_command");
}
