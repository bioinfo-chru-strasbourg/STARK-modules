...
close (HTML_OUT);
if ($address ne ""){
system ("echo \"From: SIFT <sift\@fhcrc.org>\
Subject: SIFT predictions on dbSNP\
To: $address \
\" | /opt/sfw/bin/mutt -F /opt/www/sift/htdocs/.muttrc -a $out -H /dev/stdin"); 
}
#-------------------------------------------------------------------------
exit (0);
...
sub
print_dbSNP_predictions {
	my ($dbSNP_file, %snp_hash) = @_;
	...
	print HTML_OUT "<TH ROWSPAN=2>SNP</TH><TH ROWSPAN=2>Amino acid change</TH><TH ROWSPAN=2>Protein</TH><TH ROWSPAN=2>Amino Acid</TH>\n";
	print HTML_OUT "<TH COLSPAN=4>Using orthologues in the protein alignment</TH>\n";
	print HTML_OUT "<TH COLSPAN=4>Using homologues in the protein alignment</TH></TR>\n"; 
	print HTML_OUT "<TR><TH>Prediction</TH><TH>Score</TH><TH>Median Sequence IC</TH><TH>Alignment</TH>\n";
	print HTML_OUT "<TH>Prediction</TH><TH>Score</TH><TH>Median Sequence IC</TH><TH>Alignment</TH></TR>\n";
	my $dbsnp_line;	
	while ($dbsnp_line =  <DBSNP>) {
		...
		if (exists ($snp_hash{$snpid})) {
			my ($best_hit_prot_alignment_link,
				$all_hit_prot_alignment_link) =
...
#			print HTML_OUT "found $snpid<BR>$dbsnp_line\n";
			...
			$_ = $aasub;
			/([A-Z\*])([0-9]+)([A-Z\*])/;
			my $refaa = $1; my $pos = $2; my $newaa = $3;
...
#			print HTML_OUT "<BR>score1 $score1_besthit\n";
			...
			my $prediction1_line = print_multiple_cells
				( ($prediction1_besthit, $score1_besthit)); 
			my $prediction2_line = print_multiple_cells
			...
			$refaa_prediction_line .= "<TD ROWSPAN=2>$info1_besthit" . "</TD><TD ROWSPAN=2>" . $best_hit_prot_alignment_link . "</TD>"; 
			} else {
				$refaa_prediction_line .= $no_prediction_line;
			}
			my $prediction3_line = print_multiple_cells
				(($prediction3_allhit, $score3_allhit));
			my $prediction4_line = print_multiple_cells
				(($prediction4_allhit, $score4_allhit)); 
...
#print HTML_OUT "$snpid  $info3_allhit<BR>";  	
			if ($info3_allhit > 0) {	
			$refaa_prediction_line .= "<TD ROWSPAN=2>$info3_allhit" . "</TD><TD ROWSPAN=2>" . $all_hit_prot_alignment_link . "</TD>"; 	
			} else {
				$refaa_prediction_line .= $no_prediction_line;
#				$newaa_prediction_line .= $no_prediction_line;; 
	...
	while (my ($id, $seen) = each %snp_hash )  {
		if ($seen ne "SEEN") {
			print HTML_OUT "<TR><TD>$id</TD><TD colspan=11>Not found</TD></TR>\n"; 
		}
	}

	print HTML_OUT "</TABLE><BR>\n";
	print HTML_OUT "<font color=red>* Low confidence means that the protein alignment does not have enough sequence diversity. Because the position artifically appears to be conserved, an amino acid may incorrectly predicted to be damaging.  </font><BR>" ; 
}

sub
print_multiple_cells {
...
{
	my ($protein) = @_;
	my $all_hits_alignment_file = `ls /home/sift/BLink_alignments/Predictions_ALL_HIT_Proteins/*/$protein*.msf`;

	my $best_hits_alignment_file = `ls /home/sift/BLink_alignments/Predictions_BEST_HIT_Proteins/*/$protein*.msf`;
...
#print HTML_OUT "newfile1 is $newfile1 newfile2 $newfile2<BR>\n"; 	
	my $allhits_link = "<A HREF=\"http://blocks.fhcrc.org/sift-bin/catfile.csh?" . $newfile1 . "+Alignment+PRE\" TARGET=_blank>Alignment of all hits</A>";

	my $besthits_link = "<A HREF=\"http://blocks.fhcrc.org/sift-bin/catfile.csh?" . $newfile2 . "+Alignment+PRE\" TARGET=_blank>Alignment of best hits</A>\n"; 
	return ($besthits_link, $allhits_link);
}

sub
modify_with_html_tags
{
	my ($dbsnp_line) = @_;
#	print HTML_OUT "dbsnp line $dbsnp_line";
	...
	my $index;
	foreach $index ( (7,21) ) {
		if ($fields[$index] > 3.25 && 
			($fields[$index-2] =~ /DAMAGING/ || 
				$fields[$index+5] =~ /DAMAGING/) ) {
			$fields[$index] .= "<BR><i><font color=red>Warning! Low confidence.*</font></i>"; 
		}
	} 
	my $newline;
	for (my $i = 0; $i < @fields; $i++) {
