	...
	$num_files++;
		...
		$q_loc = $q_index{$pid};
		 ...
		 if (exists $s_index{$pid}){
                        $s_loc = $s_index{$pid};
			push @jobs,("$q_loc $a_loc $s_loc $pid $outdir_partition");	
                }
                else{
                        next;
                }
...
$job_num ++;
	if ($job_num % 100 == 0){
		close OUTFILE;
		if ($job_batch_num > -1){
			$command = "qsub -P 08010 -j y -V -cwd -l \"arch=lx*64\" /usr/local/projects/SIFT/scripts/from_sift/seqs_chosen_via_median_info_from_list.pl $out";
