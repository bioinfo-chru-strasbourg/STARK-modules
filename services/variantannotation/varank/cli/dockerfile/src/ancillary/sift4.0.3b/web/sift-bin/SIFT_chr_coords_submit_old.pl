...
foreach $row (@query_result){
	...
	$CDS = @elts[20];
	$AA1_VALID = @elts[21];
	$sth_db_supp_geneinfo->execute();
        while (@rows = $sth_db_supp_geneinfo->fetchrow_array()){
                $gene_name
	...
	#system("echo $row >> $tmp/$pid.remove");
}
		...
		if (scalar @rows_CDS == 0){						# no CDS found for this coords
			if (@rows_no_CDS == 0){ 					#coord not in db
				...
				$seqs_rep = "";
                                ...
                                $result = "$rsid\t$enst\t$ensp\t$region\t$snp_type\t$codon1\t$codon2\t$AA1\t$AA2\t$AAPOS2\t$score\t$median\t$seqs_rep";
			...
			if($selected_row != -1){			#valid CDS
				...
				push @coord_arr_CDS, "$chr\t$coord1\t$coord2\t$orn_db\t$nt1\/$nt1\t$usr_comment";
        	                $result = "$rsid\t$enst\t$ensp\t$region\tSynonymous\t$codon1\t$codon1\t$AA1\t$AA1\t$AAPOS2\t$score\t$median\t$seqs_rep";
...
foreach $coord (@coord_arr_CDS) {
	...
	if (exists ($hash_enst_aapos_seen{"$enst\t$pos"})){
		...
		$enst_line          = "$coord\t$enst\t$aa1$pos$aa2\t$ensp\t$rsid\t$region\t$codon1-$codon2\t$snp_type\t$score\t$median\t$seqs_rep";
...
foreach $coord (@coord_arr_no_CDS) {
	...
	if ($aa1 eq "NA"){
        ...
        $enst_line          = "$coord\t$enst\t$subst\t$ensp\t$rsid\t$region\t$codon1-$codon2\t$snp_type\t$score\t$median\t$seqs_rep";	
        push( @enst_pred_list, $enst_line );
	print ENSTFILE "$enst_line\n";
}
close(ENSTFILE);
...
while (<ENST_PRED_FILE>) {
	...
	$table_row =
"$chrom,$coords,$orn,$alleles\t$codons\t$proteinid\t$ensp\t$subst\t$region\t$rsid\t$snp_type\t$pred\t$score\t$median\t$seqs_rep\t$usr_comment\n";
		$chrom     = "";
		$orn       = "";
		$alleles = "";
		$codons = "";
