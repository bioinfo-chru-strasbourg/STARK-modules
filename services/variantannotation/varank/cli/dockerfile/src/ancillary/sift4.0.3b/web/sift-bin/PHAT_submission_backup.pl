...
if ($names{database} ne "")
{ print DB $names{database}; }
print DB "\n";
close (DB);       
...
system("/howard/servers/btest/bin/fastaseqs $query $queryclean");
system ("/howard/servers/btest/bin/fastaseqs $dbase $dbaseclean");
...
print "<A NAME=top><H1>PHAT Results</H1></A>";
print "<PRE>";
