...
foreach $pid (@pid_arr){
	$num_files++;
		...
		$q_loc = $q_index{$pid};
        	$a_loc = $a_index{$pid};
        	$s_loc = $s_index{$pid};
        	system("qsub -P 08010 -j y -V -cwd  -o $resultdir -l \"arch=lx*64\" /usr/local/projects/GOSII/pkumar/tmp/scripts/from_sift/SIFT_prediction.csh $q_loc $s_loc $outdir_partition $pid");
        }
	else{
		$outdir_partition = "$outdir/partition_$num_dirs";
		$resultdir = "$outdir_partition\/$pid\_result";
