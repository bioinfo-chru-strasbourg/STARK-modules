...
#print "update_status($master_pid,$pid,\"Running\")<BR>";
...
system("$bin/map_choords_to_chr_bin.pl $all_chr_file $pid");

open (SNP_CHR_MAP_FILE, "$tmp/$pid.snp_chr_map_file.txt");
while (<SNP_CHR_MAP_FILE>){
        chomp;
        @elts = split /\t/, $_;
        $chr = @elts [0];
        $snp_chr_bin_file = @elts[1];
        $db_chr =DBI->connect( "dbi:SQLite:dbname=/opt/www/sift/db/Human_db/single/Human_CHR$chr.sqlite","", "", { RaiseError => 1, AutoCommit => 1 } );
        open (BIN_FILE, "$snp_chr_bin_file") || die ("Cannot open snp_chr_bin_file");
        $coord2_str = "";
	$coord1_str = "";
	while (<BIN_FILE>){
		$count ++;
		...
		#$sth_db_chr = $db_chr->prepare("select * from chr$chr where chr = \'chr$chr\' AND COORD2 = ?");
		#$sth_db_chr->execute("$coord2");
	...
	$sth_db_chr = $db_chr->prepare("select * from chr$chr where chr = \'chr$chr\' AND COORD2 in ($coord2_str) ");
	$sth_db_chr->execute();
        while (@rows = $sth_db_chr->fetchrow_array()){
		push @query_result, join("\t",@rows);
	...
	while (<BIN_FILE>){
		...
		if (scalar @rows_CDS == 0){						# no CDS found for this coords
			...
			if($selected_row != -1){			#valid CDS
				...
				print BAD_ENST_FILE "$chr\t$coord1\t$coord2\t$orn_db\t$nt1\/$nt2\n";
				next;
			}
		}
	}
...
$heading =
"Coordinates\tAlleles\tCodons\tTranscript ID\tProtein ID\tSubstitution\tRegion\tdbSNP ID\tSNP Type\tPrediction\tScore\tMedian Info\t\# Seqs at position";
...
while (<ENST_PRED_FILE>) {
	...
	$table_row =
"$chrom,$coord_begin-$coord_end,$orn\t$alleles\t$codons\t$proteinid\t$ensp\t$subst\t$region\t$rsid\t$snp_type\t$pred\t$score\t$median\t$seqs_rep\n";
		$chrom     = "";
		$orn       = "";
		$alleles = "";
		$codons = "";
...
if ($last_partition == 1){
	for ($i = $master_pid + 1; $i <= $pid; $i ++){
		system("cat $tmp/$i.combined_siftresults.predictions.table.html >> $tmp/$master_pid.combined_siftresults.predictions.table.html");
		system("cat $tmp/$i.combined_siftresults.predictions.tsv >> $tmp/$master_pid.combined_siftresults.predictions.tsv");
	}
...
sub update_status{
	...
	while (<OUTPAGE>){
			...
			$partition = "Complete set";
                        ...
                        if ($status =~ /running/i){
                                print OUTPAGE_MODIFIED "<tr BGCOLOR=\"\#DCDCDC\"><td>$partition</td><td>$job_size</td><td>$slave_pid</td><td><font color=blue>Running..</font></td><td>Not available</td></td><td>Not available</td></tr>\n";
                        }
                        elsif($status =~ /complete/i){
