...
print "Your job id is $pid and is currently running.  If your browser times out before results are shown, open this <A HREF=\"\http://blocks.fhcrc.org/sift-bin/format.pl?$pid\">link<A> in 20 minutes in a new window.  <BR><BR> Problems? Contact <A HREF=\"http://blocks.fhcrc.org/contact.html\">us<A> with your job id.\n";
...
#query_seq $queryseqfile $out $info $subst_file 0 $seq_identity_filter 
#$names{database} $address > $comments 2>/dev/null");
}
else
{
#	system ("./add_queue_entry.pl SIFT_queue $program_call $pid query_seq $queryseqfile $out $info $subst_file 0 $seq_identity_filter $names{database} $address > $comments 2> /dev/null"); 
