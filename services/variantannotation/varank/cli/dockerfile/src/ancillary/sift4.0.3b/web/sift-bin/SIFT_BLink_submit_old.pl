...
if ( $ip_counts > $upper_limit ) {
	...
	print
"Please download <A HREF=\"http://blocks.fhcrc.org/sift/SIFT.html\">SIFT software</A HREF> directly to your computer or <A HREF=\"http
://blocks.fhcrc.org/blocks-bin/contact.pl\">contact</A HREF> us so that we can help you.  Thank you for using SIFT. <BR>";
	exit;
}
...
select(STDOUT);
$| = 1;
#print "$bin/add_queue_entry.pl $bin/SIFT_queue $program_call $pid NCBI_blink $names{gi_number} $out 0 $subst_file 0 $seq_identity_filter $sequences_to_select $address > $comments 2>&1";
$queue_num = int(rand(2));
if ($queue_num == 0){
	$queue = "SIFT_queue";
}
else{
...
system(
"$bin/add_queue_entry.pl $bin/$queue $program_call $pid NCBI_blink $names{gi_number} $out 0 $subst_file 0 $seq_identity_filter $sequences_to_select $address > $comments 2>&1"
);



#system(
#"$program_call $pid NCBI_blink $names{gi_number} $out 0 $subst_file 0 $seq_identity_filter $sequences_to_select $address > $comments 2>&1"
#);
...
"<A HREF=\"/sift-bin/catfile_notitle.csh?$tmp/$pid.alignedfasta+PRE\" TARGET=_blank>Alignment in FASTA format </A> (for modification)<BR>";
	...
	print
"<i>Please check the sequences that have been chosen.  If the sequences are too diverged from your query or the alignment is questionable, we suggest
you modify the fasta-formatted file above and <A HREF=\"http://blocks.fhcrc.org/sift/SIFT_aligned_seqs_submit.html\" TARGET=_blank>resubmit</A>.</i><BR>";
...
if ($address ne ""){
        ...
        close (MESSAGE);
	my $mutt_command = "mutt -F /opt/www/sift/htdocs/.muttrc -a $tmp/$pid.alignedfasta -a $tmp/$pid.siftresults.matrix.html -a $tmp/$pid.siftresults.predictions.html";
	
	#find how many tables and attach
	for ($i = 1; $i <= $num_tables; $i++){
		$mutt_command.=" -a $tmp/$pid.aatable$i.html"
	}
	$mutt_command.=" -s \"SIFT Results for Job ID $pid\" $address <$tmp/$pid.email_message.txt";
        system("$mutt_command");
}
