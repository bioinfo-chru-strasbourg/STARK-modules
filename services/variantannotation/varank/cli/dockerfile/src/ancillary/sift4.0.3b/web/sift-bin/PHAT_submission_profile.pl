...
print "<TITLE>Profile format</TITLE>\n";
...
system("$bin/PHAT_submission_profile.csh $queryclean $txmem $nontxmem_matrix $txmem_matrix > $out 2>/dev/null");

print "</PRE>";

print "<A NAME=top><H1><center>Profile format for SWAT</center></H1></A>";
print "<PRE>";

$queryclean .=  ".prf";
unless (open(OUT, "<$queryclean")) { print "trouble opening $queryclean file"};
;
print "<PRE>";
while ($_ = <OUT>) { print $_; }
close(OUT);
