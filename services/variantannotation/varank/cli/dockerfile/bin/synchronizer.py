        ...
        if element == commons.default_pattern:
            for stark_vcf_file in vcf_files:
                if re.match(run_repository + "\/.+\/STARK\/.+\.reports\/[^.]+.final.vcf.gz", stark_vcf_file):
                    log.info(f"Syncronizing {os.path.basename(stark_vcf_file)} from \"{element}\" pattern")
                    subprocess.run(["rsync", "-rp", stark_vcf_file, run_informations["run_processing_folder_vcf_run"]])
        else:
            for pattern_vcf_file in vcf_files:
                subprocess.run(["rsync", "-rp", pattern_vcf_file, run_informations["run_processing_folder_vcf_run"]])
