...
def launch_universal_sync18(pattern):
    varank_processing_vcf_files = glob.glob(
        os.path.join(
            os.environ[
                "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_SERVICES"
            ],
            os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
            os.environ[
                "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"
            ],
            "Archives",
            "*",
            "*",
    ...
    stark_archives_all_vcf_files = glob.glob(
        os.path.join(
            os.environ[
                "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_ARCHIVES"
            ],
            "*",
            "*",
            "*",
    ...
    for stark_archives_all_vcf_file in stark_archives_all_vcf_files:
        ...
        varank_processing_run_vcf_folder = os.path.join(
            os.environ[
                "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_SERVICES"
            ],
            os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        ...
        subprocess.run(
            [
                "rsync",
                "-rp",
                stark_archives_all_vcf_file,
                varank_processing_run_vcf_folder,
            ]
        )
        ...
        if all_vcf == "*/STARK/*.reports/*.final.vcf.gz":
            ...
            for all_other_vcf_file in all_other_vcf_files:
                ...
                varank_processing_run_vcf_folder = os.path.join(
                    ...
                    os.environ[
                        "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"
                    ],
                    os.environ[
                        "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"
                    ],
...
def vcf_synchronizer(
    pattern, run_repository, varank_processing_run_vcf_folder, varank_processing_folder
):
    ...
    for stark_vcf_file in stark_vcf_files:
        if not os.path.isdir(varank_processing_run_vcf_folder):
            os.makedirs(varank_processing_run_vcf_folder)
        if re.match(
            run_repository + "\/.+\/STARK\/.+\.reports\/[^.]+.final.vcf.gz",
            stark_vcf_file,
        ):
            subprocess.run(
                ["rsync", "-rp", stark_vcf_file, varank_processing_run_vcf_folder]
            )
        ...
        if vcf == pattern[0]:
            ...
            for other_vcf_file in other_vcf_files:
                if not os.path.isdir(varank_processing_run_vcf_folder):
                    os.makedirs(varank_processing_run_vcf_folder)
                subprocess.run(
                    ["rsync", "-rp", other_vcf_file, varank_processing_run_vcf_folder]
                )
            pattern.remove(vcf)

    varank_processing_vcf_files = glob.glob(
        os.path.join(varank_processing_folder, "VCF", "*", "*.final.vcf*")
    )
    for varank_processing_vcf_file in varank_processing_vcf_files:
        os.chmod(varank_processing_vcf_file, 0o777)


def varank_processing_folder_initializer(
    varank_processing_tsv_folder, varank_processing_folder
):
    varank_processing_vcf_files_run_vcf_folder = glob.glob(
        os.path.join(varank_processing_folder, "VCF", "*", "*.final.vcf*")
    )
    varank_processing_run_vcf_folder = glob.glob(
        os.path.join(varank_processing_folder, "VCF", "*")
    )
    varank_processing_vcf_files_vcf_folder = glob.glob(
        os.path.join(varank_processing_folder, "VCF", "*.final.vcf*")
    )
    varank_processing_vcf_files_main_folder = glob.glob(
        os.path.join(varank_processing_folder, "*.final.vcf*")
    )
    other_varank_processing_vcf_files = (
        varank_processing_vcf_files_main_folder + varank_processing_vcf_files_vcf_folder
    )

    deleted_samples_file = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "configfiles",
        ...
        with open(deleted_samples_file, "r") as read_file:
            ...
            for line in read_file.readlines():
                ...
                if line[4] == os.path.basename(varank_processing_folder):
                    vcf_file_to_delete = os.path.join(
                        varank_processing_folder,
                        "VCF",
                        line[1],
                        line[0] + ".final.vcf.gz",
                    )
                    if os.path.isfile(vcf_file_to_delete):
                        ...
                        print(
                            ...
                            + "] VARANK: Deleted sample "
                            + line[0]
                            + " from run "
                            + line[1]
                            + " for this VaRank analysis"
                        )
    ...
    if len(varank_processing_vcf_files_run_vcf_folder) != 0:
        ...
        for varank_processing_run in varank_processing_run_vcf_folder:
            varank_processing_vcf_file_list = glob.glob(
                os.path.join(varank_processing_run, "*.final.vcf*")
            )
        ...
        varank_processing_vcf_default_folder = os.path.join(
            varank_processing_folder, "VCF", "DEFAULT"
        )

        for other_varank_processing_vcf_file in other_varank_processing_vcf_files:
            shutil.move(
                other_varank_processing_vcf_file, varank_processing_vcf_default_folder
            )

        varank_processing_vcf_files_default_folder = glob.glob(
            os.path.join(varank_processing_vcf_default_folder, "*.final.vcf*")
        )
        for (
            varank_processing_vcf_file_default_folder
        ) in varank_processing_vcf_files_default_folder:
            subprocess.run(
                [
                    "rsync",
                    "-rp",
    ...
    if os.path.isdir(varank_processing_tsv_folder):
        with open(
            os.path.join(varank_processing_tsv_folder, "TSVDeleting.txt"), mode="a"
        ):
            pass
        ...
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: Cleaning old TSV folder from "
            + varank_processing_folder
        )
...
def configfile_family_manager(args, run_platform_application):
    configfile_shelter = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "configfiles",
        "configfile." + run_platform_application,
    )
    default_configfile_shelter = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "configfiles",
    ...
    configfile_shelter_tmp = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "configfiles",
        "configfile." + run_platform_application + ".tmp",
    )
    default_configfile_shelter_tmp = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "configfiles",
        "configfile.default.tmp",
    )
    varank_family_tracker = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "configfiles",
        "varank_family_tracker.tsv",
    )
    configfiles_folder = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "configfiles",
    ...
    if os.path.isfile(configfile_shelter):
        ...
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: There is no specific SampleSheet for this application, adding families to the default SampleSheet"
        )
        ...
        for member in members:
            if member not in samples_name:
                error = (
                    ...
                    + "] VARANK: "
                    + member
                    + " doesn't exist in this run, please check its spelling, exiting"
                )
                print(error)
                if args.run:
            ...
            if member in all_families:
                print(
                    ...
                    + "] VARANK: "
                    + member
                    + " was not added to a new family in configfile because he has already its own family, if you want to add a new member to a family you must precise the family number in a specific application, example [fam2:SGT06]"
                )
                member_exist = True
    ...
    if member_exist is True:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] The following samples are already in the configfile and have their own family, please check your"
        ...
        for family in families:
            ...
            if is_new_member is False:
                with open(fam_conf, "a+") as write_file, open(
                    varank_family_tracker, "a+"
                ) as write_file2:
                    number = int(number) + 1
                ...
                with open(fam_conf, "r") as read_file, open(
                    tmp_fam_conf, "w"
                ) as write_file, open(varank_family_tracker, "a+") as write_file2:
                    for line in read_file.readlines():
                        line = line.strip()
...
def varank_processing_folder_configfile_manager(
    run_platform_application, varank_processing_folder, args
):
    if run_platform_application == "default":
        ...
        used_configfile = os.path.join(
            varank_processing_folder, "configfile." + run_platform_application
        )
        configfile_shelter_default = os.path.join(
            os.environ[
                "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"
            ],
            os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        ...
        used_default_configfile = os.path.join(
            varank_processing_folder, "configfile.default"
        )
        renamed_used_configfile = os.path.join(varank_processing_folder, "configfile")

    if os.path.isfile(configfile_shelter) and not os.path.isfile(
        renamed_used_configfile
    ):
        ...
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: Configfile in "
            + varank_processing_folder
            + " was synced from configfile shelter"
        )
    elif (
        os.path.isfile(configfile_shelter)
        and os.path.isfile(renamed_used_configfile)
        and not args.varank_folder
    ):
        subprocess.run(["rsync", "-rp", configfile_shelter, varank_processing_folder])
        os.rename(used_configfile, renamed_used_configfile)
        print(
            ...
            + "] VARANK: Configfile in "
            + varank_processing_folder
            + " was updated from configfile shelter"
        )
    elif not os.path.isfile(configfile_shelter) and os.path.isfile(
        renamed_used_configfile
    ):
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: Using existing configfile in "
            + varank_processing_folder
        )
    elif not os.path.isfile(configfile_shelter) and not os.path.isfile(
        renamed_used_configfile
    ):
        subprocess.run(
            ["rsync", "-rp", configfile_shelter_default, varank_processing_folder]
        )
        os.rename(used_default_configfile, renamed_used_configfile)
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: Using default configfile from configfile shelter"
        )
...
def varank_launcher(varank_processing_folder):
    ...
    print(
        "["
        + datetime.now().strftime("%y%y%m%d-%H%M%S")
        + "] VARANK: Generating TSV files, you can follow the progress in "
        + logfile
    )
    with open(logfile, "w") as f:
        subprocess.call(
            [
                ...
                '"yes"',
                "-AlamutProcesses",
                os.environ["DOCKER_VARANK_USED_THREADS"],
            ],
            stdout=f,
...
def varank_processing_folder_cleaner(
    varank_processing_tsv_folder, varank_processing_folder, args
):
    ...
    with open(
        os.path.join(varank_processing_tsv_folder, "TSVCopyComplete.txt"), mode="a"
    ):
        pass
...
def generate_non_redundant(varank_processing_tsv_folder, platform_application):
    all_variants_ranking_by_var = os.path.join(
        varank_processing_tsv_folder, "*_allVariants.rankingByVar.tsv"
    )
    all_variants_ranking_by_var_files = glob.glob(all_variants_ranking_by_var)
    all_variants_ranking_by_var_concat = os.path.join(
        varank_processing_tsv_folder, "temp_concat_file.tsv"
    )
    ...
    with open(all_variants_ranking_by_var_concat, "r"):
        df = pandas.read_csv(
            all_variants_ranking_by_var_concat,
            sep="\t",
            header=[0],
            low_memory=False,
        ...
        if re.match(r"DIAG\.WES_.*", platform_application):
            df["gnomadAltFreq_popmax"] = df["gnomadAltFreq_popmax"].replace(
                {",": "."}, regex=True
            )
    ...
    print(
        "["
        + datetime.now().strftime("%y%y%m%d-%H%M%S")
        + "] VARANK: Non-redondant file was generated"
    )


def run_varank_results_provider(
    varank_processing_tsv_folder,
    run_depository,
    run_repository,
    varank_processing_folder,
    run_platform,
    ...
    varank_processing_folder_tsv_files = glob.glob(
        os.path.join(varank_processing_tsv_folder, "*")
    )
    run_repository_sample_folder_list = glob.glob(os.path.join(run_repository, "*", ""))

    varank_processing_folder_logfile = os.path.join(
        varank_processing_folder, "VaRank.log"
    )
    ...
    varank_renamed_logfile_depository = os.path.join(
        run_depository,
        "VARANK." + datetime.now().strftime("%Y%m%d-%H%M%S") + ".VaRank.log",
    )
    varank_renamed_logfile_repository = os.path.join(
        run_repository,
        "VARANK." + datetime.now().strftime("%Y%m%d-%H%M%S") + ".VaRank.log",
    )
    ...
    non_redundant_renamed_file_depository = os.path.join(
        run_depository,
        "VARANK." + datetime.now().strftime("%Y%m%d-%H%M%S") + ".Non_Redondant.tsv",
    )
    non_redundant_renamed_file_repository = os.path.join(
        run_repository,
        "VARANK." + datetime.now().strftime("%Y%m%d-%H%M%S") + ".Non_Redondant.tsv",
    )
    ...
    for tsv_file in reversed(varank_processing_folder_tsv_files):
        if os.path.basename(tsv_file).startswith("Non_Redondant"):
            ...
            for sample_folder in run_repository_sample_folder_list:
                ...
                varank_repository = os.path.join(
                    run_repository, os.path.basename(sample_folder), "VARANK", ""
                )
                varank_depository = os.path.join(
                    run_depository, os.path.basename(sample_folder), "VARANK", ""
                )
                ...
                run_repository_sample_folder_varank_folder = os.path.join(
                    varank_repository, os.path.basename(sample_folder) + "_" + date, ""
                )
                run_depository_sample_folder_varank_folder = os.path.join(
                    varank_depository, os.path.basename(sample_folder) + "_" + date, ""
                )
                    ...
                    if test == os.path.basename(sample_folder):
                        subprocess.run(
                            [
                                ...
                                "-rp",
                                tsv_file,
                                run_repository_sample_folder_varank_folder,
                            ]
                        )
                        subprocess.run(
                            [
                                ...
                                "-rp",
                                tsv_file,
                                run_depository_sample_folder_varank_folder,
                            ]
                        )
    ...
    os.rename(
        os.path.join(run_depository, "VARANKComplete.txt"), varank_copy_complete_file
    )
...
def launch_run_analysis(args):
    ...
    varank_processing_folder = os.path.join(
    ...
    varank_processing_run_vcf_folder = os.path.join(
        varank_processing_folder, "VCF", os.path.basename(run_repository)
    )
    ...
    print(
        "["
        + datetime.now().strftime("%y%y%m%d-%H%M%S")
        + "] VARANK: Your analysis ended well, you can check results in "
        + results
    )
...
def launch_folder_analysis(args):
    ...
    if folder.startswith(
        ...
        varank_application_paths = glob.glob(
            os.path.join(
                os.environ[
                    "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_SERVICES"
                ],
        ...
        for varank_application_path in varank_application_paths:
            ...
            if (
                not folder_platform == varank_application_path[6]
                and not folder_application == varank_application_path[7]
            ):
                check = False
            if (
                folder_platform == varank_application_path[6]
                and folder_application == varank_application_path[7]
            ):
                check = True
                break
        ...
        if check is False:
            print(
                "["
                + datetime.now().strftime("%y%y%m%d-%H%M%S")
                + "] VARANK: You are not allowed to launch a VaRank analysis from here "
                + folder
                + ", exiting"
            )
            exit()
        ...
        varank_processing_folder_configfile_manager(
            folder_platform_application, varank_processing_folder
        )
        varank_launcher(varank_processing_folder)
        ...
        generate_non_redundant(
            varank_processing_tsv_folder, folder_platform_application
        )
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
        ...
        varank_processing_folder_configfile_manager(
            folder_platform_application, varank_processing_folder, args
        )
        varank_launcher(varank_processing_folder)
        logfile_checker(varank_processing_folder, args, "None", "None")
        varank_processing_folder_cleaner(
...
def generated_tsv_checker(varank_processing_tsv_folder, args):
    ...
    if len(tsv_files) <= 2:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: No TSV files were generated during the analysis, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    for tsv_file in tsv_files:
        ...
        if os.stat(tsv_file).st_size == 0:
            error = (
                "["
                + datetime.now().strftime("%y%y%m%d-%H%M%S")
                + "] VARANK: There are empty TSV files in "
                + varank_processing_tsv_folder
                + ", exiting"
            )
            print(error)
        ...
        if count_family_barcode > 1:
            error = (
                "["
                + datetime.now().strftime("%y%y%m%d-%H%M%S")
                + "] VARANK: Results in *rankingByVar are double for a sample, you need to delete Alamut folder and launch the run again, exiting"
            )
            print(error)
            if args.run:
                error_log_writer(error)
...
def vcf_synchronization_checker(varank_processing_folder):
    ...
    # Checking if there are empty vcf files in vcf folder
        ...
        vcf_files = subprocess.check_output(
            ["find", varank_processing_folder, "-name", "*.final.vcf.gz"]
        )[:-1].decode("utf-8")
        ...
        for vcf_file in vcf_files_list:
            if os.stat(vcf_file).st_size == 0:
                error = (
                    "[" + datetime.now().strftime("%y%y%m%d-%H%M%S"),
                    "] VARANK: There is no VCF files in " + vcf_folder + ", exiting",
                )
                print(error)
                error_log_writer(error)
                exit()
...
def path_checker(args, path):
    ...
    run_repository_folder = os.environ[
        "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_REPOSITORY"
    ]
    run_depository_folder = os.environ[
        "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_DEPOSITORY"
    ]
    ...
    if not os.path.isdir(path) and path.startswith(run_depository_folder):
        print(
            ...
            + "] VARANK: Specified "
            + path
            + " folder doesn't exist, maybe it was sent to Archives ? Creating new folder with all sample subfolders"
        )
        run_repository_sample_folder_list = glob.glob(os.path.join(args.run, "*", ""))
        os.makedirs(path, 0o775)
        for sample_folder in run_repository_sample_folder_list:
            sample_folder = re.sub(
                run_repository_folder, run_depository_folder, sample_folder
            )
...
def pattern_checker(pattern, run_repository, args):
    ...
    for element in pattern:
        ...
        if len(vcf_files) == 0 and element != "*/*.final.vcf.gz":
            print(
                "["
                + datetime.now().strftime("%y%y%m%d-%H%M%S")
                + "] VARANK: There is no vcf files with the following pattern in this run : "
                + element
            )
            ...
            error = (
                "["
                + datetime.now().strftime("%y%y%m%d-%H%M%S")
                + "] VARANK: There is no vcf files with the STARK analysis pattern in this run : "
                + element
            )
            error_log_writer(error)
            check1 = True
        ...
        for vcf_file in vcf_files:
            if os.stat(vcf_file).st_size == 0:
                error = (
                    "["
                    + datetime.now().strftime("%y%y%m%d-%H%M%S")
                    + "] VARANK: The following vcf file is empty : "
                    + vcf_file
                )
                error_log_writer(error)
                check2 = True
    ...
    if check1 is True:
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: There is no vcf files with the following pattern in this run, exiting"
        )
        if check2 is False:
            exit()
    if check2 is True:
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: There are empty vcf files in your run, exiting"
        )
        exit()
...
def environment_checker(args):
    ...
    if not os.path.isfile(
        os.path.join(
            os.environ[
                "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_DATABASES"
            ],
            "OMIMannotations",
            "current",
            "OMIMannotations.tsv.gz",
        )
    ):
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: OMIM database is missing in the ExtAnn VaRank folder, please check docker-compose.yml file, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if not os.path.isfile(
        ...
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: ALAMUT database is missing in the database folder, please check docker-compose.yml file, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if (
        ...
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: OMIM database is empty in the ExtAnn VaRank folder, please check docker-compose.yml file, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if (
        ...
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: ALAMUT database is empty in the database folder, please check docker-compose.yml file, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if not os.path.isdir(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_REPOSITORY"]
    ):
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: STARK repository folder is missing inside the container, please check your docker configuration, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
        ...
        exit()
    if not os.path.isdir(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_DEPOSITORY"]
    ):
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: STARK depository folder is missing inside the container, please check your docker configuration, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if not os.path.isdir(
        ...
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: VARANK config folder is missing inside the container, please check your docker configuration, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if (
        ...
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: STARK repository folder is empty inside the container, please check your docker configuration, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if (
        ...
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: VARANK config folder is empty inside the container, please check your docker configuration, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
...
def varank_config_folder_checker(args):
    ...
    deleted_samples_config = os.path.join(
        configfiles_folder, "deleted_samples_config.tsv"
    )
    extann_config_file = os.path.join(configfiles_folder, "extann_config_file.tsv")
    varank_family_tracker = os.path.join(
        configfiles_folder, "varank_family_tracker.tsv"
    )
    ...
    alamut_license_folder = os.path.join(
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_CONFIG"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_MODULE_NAME"],
        os.environ["DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_SUBMODULE_NAME"],
        "alamut-batch-license",
    ...
    if not os.path.isfile(varank_config_initialized):
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: For a first launch, you need to launch manually VaRank with --configure option, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if not os.path.isfile(configfile_default):
        shutil.copy(
            os.path.join(os.environ["VARANK"], "configfile"),
            os.path.join(configfiles_folder, "configfile.default"),
        )
    ...
    if not os.path.isfile(non_redundant_config):
        shutil.copy(
            os.path.join(os.environ["VARANK"], "config", "non_redundant_config.txt"),
            configfiles_folder,
        )
    ...
    if not os.path.isfile(deleted_samples_config):
        shutil.copy(
            os.path.join(os.environ["VARANK"], "config", "deleted_samples_config.tsv"),
            configfiles_folder,
        )
    ...
    if not os.path.isfile(extann_config_file):
        shutil.copy(
            os.path.join(os.environ["VARANK"], "config", "extann_config_file.tsv"),
            configfiles_folder,
        )
    ...
    if not os.path.isfile(varank_family_tracker):
        shutil.copy(
            os.path.join(os.environ["VARANK"], "config", "varank_family_tracker.tsv"),
            configfiles_folder,
        )
    ...
    if not os.path.isdir(alamut_license_folder):
        ...
        if not os.path.isfile(alamut_license):
            error = (
                "["
                + datetime.now().strftime("%y%y%m%d-%H%M%S")
                + "] VARANK: Using default alamut license file within the installation folder, you need to add your specific license in "
                + alamut_license
                + ", exiting"
            )
            subprocess.run(
                [
                    "rsync",
                    "-rp",
                    os.path.join(os.environ["ALAMUT"], "alamut-batch.ini"),
                    alamut_license,
                ]
            )
            print(error)
...
def configfile_checker(configfiles_folder, args):
    ...
    for configfile in configfiles_list:
        family_list = []
        tmp_configfile = os.path.join(
            os.path.dirname(configfile), "tmp" + os.path.basename(configfile)
        )
            ...
            for line in read_file.readlines():
                    ...
                    if os.path.basename(configfile) != "configfile.default":
                        ...
                        if len(extann_file_list) == 0 and args.configure:
                            print(
                                "["
                                + datetime.now().strftime("%y%y%m%d-%H%M%S")
                                + "] VARANK: Not using any additional extann(s) file(s) for "
                                + configfile
                            )
                        for extann_file in reversed(extann_file_list):
                            if (
                                ...
                                error = (
                                    "["
                                    + datetime.now().strftime("%y%y%m%d-%H%M%S")
                                    + "] VARANK: Not found additional extann(s) file(s) at "
                                    + extann_file
                                    + "directory, please check your configfile, exiting"
                                )
                                print(error)
                                if args.run:
                                    error_log_writer(error)
                ...
                if re.match(r"#-proxyUser:", line):
                    if re.search(r'"[a-zA-Z0-9]+"', line):
                        line = re.sub(
                            '"[a-zA-Z0-9]+"', '"' + os.environ["PROXY_USER"] + '"', line
                        )
                    else:
                ...
                if re.match(r"#-proxyPasswd:", line):
                    if re.search(r'"[a-zA-Z0-9]+"', line):
                        line = re.sub(
                            '"[a-zA-Z0-9]+"',
                            '"' + os.environ["PROXY_PASSWD"] + '"',
                            line,
                        )
                    else:
                        line = re.sub(
                            '""', '"' + os.environ["PROXY_PASSWD"] + '"', line
                        )
                    line = re.sub("#", "", line)
                    write_file.write(line + "\n")
                    writted = True
                ...
                if re.match(r"#-proxyPort:", line):
                    if re.search(r'"[a-zA-Z0-9]+"', line):
                        line = re.sub(
                            '"[a-zA-Z0-9]+"', '"' + os.environ["PROXY_PORT"] + '"', line
                        )
                    else:
...
def logfile_checker(varank_processing_folder, args, run_depository, run_repository):
    ...
    if varank_error is True:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: There is an unexpected error in the VaRank analysis, you must check its log, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
        ...
        for check in df.duplicated(subset=["variantID"]):
            if check is True:
                error = (
                    "["
                    + datetime.now().strftime("%y%y%m%d-%H%M%S")
                    + "] VARANK: There are duplicated variantID in your non-redundant file, exiting"
                )
                print(error)
                error_log_writer(error)
                exit()
        ...
        for line in read_file:
            ...
            if line == "licenseKey=...":
                error = (
                    "["
                    + datetime.now().strftime("%y%y%m%d-%H%M%S")
                    + "] VARANK: There is no licenseKey defined in the alamut-batch.ini file, exiting"
                )
                print(error)
                if args.run:
                    error_log_writer(error)
            ...
            if re.search(r"(Institution=[A-Za-z]+)", line):
                print(
                    "["
                    + datetime.now().strftime("%y%y%m%d-%H%M%S")
                    + "] VARANK: licenseKey is valid for user",
                    line.strip("Institution="),
                )
            if re.search("File=", line):
                line = line.strip("File=")
                if line == os.path.join(
                    os.environ[
                        "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARANK_FOLDER_DATABASES"
                    ],
                    "alamutDB",
                    ...
                    print(
                        "["
                        + datetime.now().strftime("%y%y%m%d-%H%M%S")
                        + "] VARANK: AlamutDB path is valid in the license file"
                    )
                else:
                    error = (
                        "["
                        + datetime.now().strftime("%y%y%m%d-%H%M%S")
                        + "] VARANK: Wrong path for the AlamutDB in the alamut-batch.ini file, exiting"
                    )
                    print(error)
                    if args.run:
                        error_log_writer(error)
...
def args_checker(args):
    ...
    if args.varank_folder and args.pattern:
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --varank_folder and --pattern together, please use only one of them, exiting"
        )
        exit()
    if args.varank_folder and args.allsync18:
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --varank_folder and --allsync18 together, please use only one of them, exiting"
        )
        exit()
    if args.varank_folder and args.configure:
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --varank_folder and --configure together, please use only one of them, exiting"
        )
        exit()
    if args.pattern and args.configure:
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --pattern and --configure together, please use only one of them, exiting"
        )
        exit()
    if args.run and args.configure:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --run and --configure together, please use only one of them, exiting"
        )
        print(error)
        error_log_writer(error)
        exit()
    if args.run and args.allsync18:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --run and --allsync18 together, please use only one of them, exiting"
        )
        print(error)
        error_log_writer(error)
        exit()
    if args.pattern and not args.run and not args.allsync18:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --pattern alone, exiting"
        )
        print(error)
        error_log_writer(error)
        exit()
    if args.varank_folder and args.pattern and args.allsync18:
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --varank_folder, --pattern together and --allsync19 together, please use only"
            " one of them, exiting"
        )
        exit()
    if args.run and args.allsync18 and args.pattern:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You cannot use --run, --allsync18 and --pattern together, please use only one of them, exiting"
        )
        print(error)
        error_log_writer(error)
        exit()
    ...
    if arg_check is False:
        error = (
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You need to enter a parameter, type -h to see help, exiting"
        )
        print(error)
        if args.run:
            error_log_writer(error)
    ...
    if args.varank_folder and not os.path.isabs(args.varank_folder):
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You have not specified an absolute path for --varank_folder argument, exiting"
        )
        exit()
    ...
    if args.run and not os.path.isabs(args.run):
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: You have not specified an absolute path for --run argument, exiting"
        )
        exit()
    ...
    if args.varank_folder and not os.path.isdir(args.varank_folder):
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: Specified VaRank analysis folder doesn't exist, exiting"
        )
        exit()
    if args.run and not os.path.isdir(args.run):
        print(
            "["
            + datetime.now().strftime("%y%y%m%d-%H%M%S")
            + "] VARANK: Specified VaRank analysis run folder doesn't exist, exiting"
        )
        exit()
        ...
        for family in args.family:
            if not re.match(r"\[[:a-zA-Z0-9,]+\]", family):
                error = (
                    "["
                    + datetime.now().strftime("%y%y%m%d-%H%M%S")
                    + "] VARANK: Error in families format, it must be [sample1,sample2,sample3] [sample4, sample5] for example, exiting"
                )
                print(error)
                if args.run:
                    error_log_writer(error)
                ...
                if len(family) > 1:
                    ...
                    if result is False:
                        error = (
                            "["
                            + datetime.now().strftime("%y%y%m%d-%H%M%S")
                            + "] VARANK: Error in families format, you are adding samples in pre-existing families you are not allowed to specify many families in one group, you need to declare a new group for each family update"
                        )
                        print(error)
                        if args.run:
                            error_log_writer(error)
...
def parse_args():
    ...
    parser.add_argument(
        "-as18",
        "--allsync18",
        help="Used to rsync all VCF files from all stark18 project to your VaRank processing directory, no argument required must be used alone",
        action="store_true",
    )
    parser.add_argument(
        "-fam",
        ...
        type=str,
        nargs="+",
        help="You can pass families to add them to the configfile, the format is the following : -fam [SGT01,SGT02,SGT03] [SGT04,SGT05]",
    )
    parser.add_argument(
        "-cfg",
        "--configure",
        help="Use this argument the very first time you launch VaRank to configure all the files and folders",
        action="store_true",
    )
