...
def launch_run(args):
    ...
    run_informations = {
        ...
        "run_depository": osj(
            os.environ[
                "DOCKER_MODULE_VARIANTANNOTATION_SUBMODULE_VARIANTANNOTATION_FOLDER_DEPOSITORY"
            ],
    ...
    variantannotation_running_log = osj(run_repository, "VARIANTANNOTATIONRunning.txt")

    with open(variantannotation_running_log, "w") as write_file:
        pass

    synchronizer.processing_folder_vcf_synchronizer(run_informations)
    howard_processing.initialisation(run_informations)
    ...
    log.info(
        f"variantannotation howard analysis for run {run_informations['run_name']} ended well"
    )
