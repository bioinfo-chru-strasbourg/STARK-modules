...
# Usage
function usage {
        echo "#USAGE: $(basename $0) --run =<RUN>|--cqi=<CQI_SAMPLE>|--genes=<GENES>|--vaf=<VAF,FLOAT,:ALLELE>|--cqigz=<CQI_VCF>|--set=<SET> [options...]";
...
ARGS=$(getopt -o "r:c:g:a:q:s:vdnh" --long "run:,cqi:,genes:,vaf:,cqigz:,set:,verbose,debug,release,help" -- "$@" 2> /dev/null)
if [ $? -ne 0 ]; then
        :
fi;
PARAM=$@
	...
	if [ ! -f $RUN/CQIComplete.txt ]; then
		...
		if [ "$REG" = false ]; then
			...
			if [ $CQI_SAMPLE = "TeCoriell" ]; then
				...
				gunzip -c ${LINE} > $CQI/CQI_VCF.vcf 2>/dev/null
				CQI_VCF=$CQI/CQI_VCF.vcf
				
			else
				CQI_SAMPLE=$(basename -s .vcf.gz ${LINE})
				CQI=$RUN/$CQI_SAMPLE/CQI
				mkdir -p $CQI
		...
		if [ "$REG" = true ]; then 
			...
			GENES=$(find $RUN/$CQI_SAMPLE -name $CQI_SAMPLE.bed) 
			#GENES=/home1/L_PROD/NGS/BAS/lamouchj/input/CQI_REF/HUSHEMATO_XTHS_V1_1.manifest.genes
			echo "GENES=$GENES"
		fi;
