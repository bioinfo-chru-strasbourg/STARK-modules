...
SCRIPT_DESCRIPTION="STARK Comparison of 2 VCF"
...
# Usage
function usage {
        echo "#USAGE: $(basename $0) --run =<RUN>|--cqi=<CQI_SAMPLE>|--genes=<GENES>|--vaf=<VAF,FLOAT,:ALLELE>|--cqigz=<CQI_VCF>|--set=<SET>|--json=<JSON> [options...]";
...
ARGS=$(getopt -o "r:c:g:a:q:s:j:vdnh" --long "run:,cqi:,genes:,vaf:,cqigz:,set:,json:,verbose,debug,release,help" -- "$@" 2> /dev/null)
if [ $? -ne 0 ]; then
        :
fi;
PARAM=$@
...
##ALL information coming from SampleSheet
SS=$(find $RUN -name "*.SampleSheet.csv" -print -quit)
for ELEM in $(grep "CQI#" $SS); do
	CQI_SAMPLE=$(echo $ELEM | cut -d ',' -f1)
	echo "[#INFO] SAMPLE as CQI: $CQI_SAMPLE"
	for ITEM in $(echo ${ELEM##*,} | cut -d '!' -f1 | cut -d '#' -f2-); do
		...
		for LAST in "${ADDR[@]}"; do 
			...
			if [ ! -f $RUN/CQIComplete.txt ]; then
				...
				if [ "$REG" = false ]; then
					...
					cat CQI.json | jq -cr '.CQI | .[] | .name'
					if [ $? -eq 0 ]; then
						echo "[#INFO] Parsing JQ Extract VCF"
						JFILE=$(cat $JSON | jq -cr '.CQI | . [] | select( .name == "$CQI_SAMPLE" ) | .VCF')
					else 
						echo "[ERROR] $CQI_SAMPLE not in list EXIT"
						exit 2
						
					#for RP/BBS ONLY tests
					if [ $CQI_SAMPLE = "TeCoriell" ]; then
						CQI=$RUN/$CQI_SAMPLE/CQI
						mkdir -p $CQI
						...
						CQI=$RUN/$CQI_SAMPLE/CQI
						mkdir -p $CQI
						RES=$(find $RUN/$CQI_SAMPLE -name $CQI_SAMPLE.final.vcf.gz)
						gunzip -c $RES > $CQI/SAMPLE_VCF.vcf 2>/dev/null
						SAMPLE_VCF=$CQI/SAMPLE_VCF.vcf
				...
				if [ "$REG" = true ]; then 
					...
					GENES=$(find $RUN/$CQI_SAMPLE -name $CQI_SAMPLE.genes ) 
					echo "GENES=$GENES"
				fi;
				
				
				touch $CQI/compare_vcf.log
				LOG=$CQI/compare_vcf.log
				...
				for VCF in "${ARRAY[@]}"; do
					...
					bcftools view --exclude-types indels -O v -o $CQI/SNV/$(basename -- $VCF) $VCF.gz
				...
				for FOLDER in "${ARRAY[@]}"; do 
					...
					if [ ! -z "${GENES}" ]; then
						echo "Number of variants after bed filtering VCF REF: $(cat $CQI_VCF_OR | grep -v "^#" | wc -l)" >> $OUTPUT
						echo "Number of variants after bed filtering VCF STARK: $(cat $SAMPLE_VCF_OR | grep -v "^#" | wc -l)" >> $OUTPUT
						printf "\n" >> $OUTPUT
					fi;
					...
					bcftools isec -c all $CQI_VCF $SAMPLE_VCF -p $FOLDER/bcftools_isec 2>>$LOG
