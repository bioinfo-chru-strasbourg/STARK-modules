...
def startService(run, serviceName, serviceDockerImage, JSON, env, cqiFolder, groupInput, genome, serviceDockerImageRtg, cqiBin):
	...
	p = subprocess.Popen("find -L "+run+" -maxdepth 1 -type d", stdout=subprocess.PIPE, shell=True)
	out = p.stdout.readlines()
	folder = list(map(str, out))
	print(folder)
	for sample in folder:
		print("SAMPLE="+sample)
		sample = sample.replace("\\n'", "")
		#print(os.path.basename(sample))
		if os.path.basename(sample) == 'TeCoriell' or os.path.basename(sample) == 'HORIZON' or os.path.basename(sample) == 'ACCROMETRIX':
			cqiName = os.path.basename(sample)
			print("CQI="+cqiName)
			break
		else:
			continue
	...
	if serviceName == "CQI":
		cmd = "docker run -ti --name="+containerName+" --env-file="+env+" -v "+cqiFolder+":"+cqiFolder+" -v "+run+":"+run+" -v "+cqiBin+":"+cqiBin+" "+serviceDockerImage+" bash -c 'source activate variant && /home1/BAS/lamouchj/CQI/bin/compare_vcf_1.1.sh --run="+os.path.basename(run)+" --genes="+bed+" --cqi="+cqiName+" --cqigz="+parseJsonCQI(JSON, os.path.basename(sample))+"'"
	...
	while True:
		if os.path.isfile(run+'/'+'COMPAREComplete.txt'):
			...
			rtg = "docker run -dti --rm --name="+name+" --entrypoint"+" /bin/bash"+" --env-file="+env+" -v "+run+":"+run+" -v "+genome+":"+genome+" -v "+cqiBin+":"+cqiBin+" "+serviceDockerImageRtg+" /home1/BAS/lamouchj/CQI/bin/rtg_vcf.sh --cqi="+cqiName+" --run="+run
