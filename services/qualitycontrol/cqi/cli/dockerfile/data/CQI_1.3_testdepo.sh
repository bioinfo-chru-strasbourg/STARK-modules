...
# Usage
function usage {
        echo "#USAGE: $(basename $0) --run =<RUN>|--cqi=<CQI_SAMPLE>|--genes=<GENES1,GENES2,...>|--vaf=<VAF,FLOAT,:ALLELE>|--cqigz=<CQI_VCF>|--set=<SET>|--json=<JSON>|--genome=<GENOME> [options...]";
		...
		if grep -q "CQI" $SAMPLE/$CQI_SAMPLE.tag 2>/dev/null; then
			...
			for TAG in "${FULL_TAG[@]}"; do
				...
				if [ "$REG" = true ]; then 
					...
					elif [[ -n $(find $RUN/$CQI_SAMPLE -name $CQI_SAMPLE.*.manifest.*.genes -print -quit) ]]; then 
						GENES=$(find $RUN/$CQI_SAMPLE -name $CQI_SAMPLE.*.manifest.*.genes -print -quit)
					else
				...
				### Copy to depository for each CQI folder
				#Create depository folder
				chmod -R 777 $CQI
				DEPO=/home1/data/current/output/depository/$(echo $CQI | cut -d '/' -f7-)
				mkdir -p $DEPO
				echo "[#INFO] Copy CQI: $TAG in $DEPO"
				rsync -rvt $CQI/* $DEPO
				
			done; #IF multiple tag CQI for one sample
			#GENES=""
			#remove intermediate vcf
			#rm -f $CQI/CQI_VCF* $CQI/SAMPLE* $CQI/ANNEX/*.vcf.gz $CQI/ANNEX/*/CQI_VCF* $CQI/ANNEX/*/SAMPLE_VCF* $CQI/ANNEX/*/final*.vcf  #/home1/BAS/lamouchj/CQI/bin/concordance*.txt
...
### Copy to depository check with Antony loop for each CQI folder
#Create depository variable
#DEPO=/home1/data/current/output/depository/$(echo $CQI | cut -d '/' -f7-)
#mkdir -p $DEPO
#rsync -rvt $CQI/* $DEPO
