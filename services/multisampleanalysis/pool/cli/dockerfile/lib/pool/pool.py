...
def add_option(config, option, name, group, option_type, required=False):
    ...
    if required and name not in config[group]:
        print(
            "[ERROR] - please define option " + name + "=<" + option_type + "> ",
            file=sys.stderr,
        )
        sys.exit()
    return config
...
def pool(config, output, jobs, dryrun, dag):
    ...
    command = (
        "/usr/local/lib/miniconda3/bin/snakemake --snakefile "
        + current_dir
        + "/../snakemake/Snakefile --configfile "
        + config
        + " --directory "
        + output
        + " --jobs "
...
def main():
    ...
    group0.add_argument(
        "-t", "--tmp", type=str, help="The temporary folder", dest="tmp"
    )
    group0.add_argument(
        "-s",
        "--sampleVcf",
        help="The sample vcf files",
        type=str,
        dest="sampleList",
        required=True,
    )
    group0.add_argument(
        "-p",
        "--pool",
        help="pool vcf and bam files: poolName:poolVcf:poolBam, example: POOL_F,A.vcf,A.bam",
        type=str,
        dest="poolList",
        required=True,
    )
    ...
    group0.add_argument(
        "-g", "--genome", help="analysis genome file", type=str, dest="genome"
    )
    group0.add_argument(
        "-n",
        "--padding",
        help="The padding used for the variant calling",
        type=str,
        dest="padding",
    )
    group0.add_argument(
        "-c", "--config", help="The json config file", type=str, dest="config"
    )
    ...
    group1.add_argument(
        "--bcftools", type=str, help="The path to the bcftools executable"
    )
    ...
    group2.add_argument(
        ...
        "--jobs",
        type=int,
        help="number of parallel cores to use (default: nbCPUs -1)",
        dest="jobs",
    )
    group2.add_argument(
        "-d", "--dag", action="store_true", help="write dag", dest="dag"
    )
    group2.add_argument(
        "-y", "--dryrun", action="store_true", help="dry run", dest="dryrun"
    )
