...
def find_any_samplesheet(runDir, fromResDir=False):
    ...
    """
    p = subprocess.Popen(
        "find -L " + runDir + " -maxdepth 3 -name *SampleSheet.csv",
...
def get_sample_list_from_samplesheet(samplesheetPath):
    ...
    """
    assert (
        samplesheetPath != "NO_SAMPLESHEET_FOUND"
...
def createEmptyVcfAndBam(pool, dockerOutputDir, sampleList, runDir, bcftools, samtools):
    ...
    vcfCompressed = bcftoolsCompress(
        osj(
            runDir,
            sampleList[0],
            "STARK",
            sampleList[0] + ".reports",
            sampleList[0] + ".final.vcf",
        ),
        dockerOutputDir,
    )
    cmd = (
        ...
        + vcf
        + " -O v "
        + osj(runDir, sampleList[0], sampleList[0] + ".final.vcf.gz")
    )
    subprocess.call(cmd, shell=True)
    cmd = (
        samtools
        ...
        + bam
        + " "
        + osj(runDir, sampleList[0], "STARK", sampleList[0] + ".bwamem.bam")
    )
    subprocess.call(cmd, shell=True)
    ...
    for s in sampleList:
        if os.path.exists(osj(dockerOutputDir, s + ".final.vcf.gz")):
            ...
            shutil.copyfile(
                ...
                osj(resDir, s + ".final.vcf.gz"),
            )
            shutil.copyfile(
                osj(dockerOutputDir, s + ".final.vcf.gz.tbi"),
                osj(resDir, s + ".final.vcf.gz.tbi"),
            )
            shutil.copyfile(
                osj(dockerOutputDir, ".log", s + ".bcftools.log"),
                osj(logDir, s + ".bcftools.log"),
            )
            shutil.copyfile(
                osj(dockerOutputDir, ".log", s + ".howard.log"),
                osj(logDir, s + ".howard.log"),
            )
            ...
            writeErrorLog(
                s,
                runDir,
                "[ERROR] Missing final VCF file. Are the input BAM & VCF correct?",
            )


def launchAnalysis(
    sampleList, key, runDir, dockerOutputDir, genome, bcftools, samtools
):
    vcfList = []
    poolFStr = "init"
    poolMStr = "init"
    ...
    for pool in key.split("#"):
        if os.path.exists(osj(runDir, pool, "STARK", pool + ".tag")):
            if is_pool(runDir, pool, "SEX#M"):
                ...
                for s in sampleList:
                    writeErrorLog(
                        s, runDir, "[ERROR] Can't find " + pool + " sample's sex."
                    )
                return "[ERROR] Can't find " + pool + " sample's sex."
        else:
            for s in sampleList:
                writeErrorLog(
                    s, runDir, "[ERROR] Specified " + pool + " sample not found."
                )
    ...
    # assert bed != "init"
    if poolFStr == "init":
        poolFStr = createEmptyVcfAndBam(
            "POOL_F", dockerOutputDir, sampleList, runDir, bcftools, samtools
        )
        # for s in sampleList:
        # writeErrorLog(s, runDir, "[ERROR] Missing POOL_F for POOL analysis.")
        # return "[ERROR] Missing POOL_F for POOL analysis."
    if poolMStr == "init":
        poolMStr = createEmptyVcfAndBam(
            "POOL_M", dockerOutputDir, sampleList, runDir, bcftools, samtools
        )
        # for s in sampleList:
    ...
    cmd = (
        "python /app/lib/pool/pool.py sample -o "
        + dockerOutputDir
        + ' -s "'
        + ",".join(vcfList)
        + '" -p "'
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-b",
        "--bcftools",
        help="path to bcftools bin",
        type=str,
        dest="bcftools",
        default="/STARK/tools/bcftools/current/bin/bcftools",
    )
    parser.add_argument(
        "-s",
        "--samtools",
        help="path to samtools bin",
        type=str,
        dest="samtools",
        default="/STARK/tools/samtools/current/bin/samtools",
    )
