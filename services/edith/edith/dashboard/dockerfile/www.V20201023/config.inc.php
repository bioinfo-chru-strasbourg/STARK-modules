...
foreach ($module_folders as $module_folder=>$module_array) {
	...
	foreach ($module_obj->{'services'} as $service=>$service_infos) {
		...
		# Service container
		$service_container=$service_infos->{'container'}!=""?$service_infos->{'container'}:"stark-module-$module_info_code-submodule-$service_submodule-service-$service_code";
		...
		$module_obj->{'submodules'}->{$service_submodule}->{'services'}->{$service}=&$module_obj->{'services'}->{$service};
